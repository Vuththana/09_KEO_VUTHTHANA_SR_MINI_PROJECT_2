...
export const categoriesResponse = async () => {
  const res = await fetch(`${process.env.NEXT_PUBLIC_AUTH_BASE_URL}/categories`, {
...
export const ESSENTIALS_TABS = { categoryId: 0, name: "All" }
export function filterProductsByEssentialsTab(list, selectedCategory) {
  ...
  return list.filter((p) => p.categoryId === selectedCategory.categoryId);
}

export function getCategoryLabel(categoriesList, categoryId) {
  if (!categoriesList || !Array.isArray(categoriesList)) {
    return `Category ${categoryId}`;
  }
  
  const c = categoriesList.find((x) => x.categoryId === categoryId);
